// Smoke checks for rate-loop-v2.html — the kj sketch resolved.
'use strict';
const fs=require('fs'), path=require('path'), cp=require('child_process'), os=require('os');

const file=path.join(__dirname,'rate-loop-v2.html');
...
// upstairs' extra seam is not the loop's: the zero edges (pushbrooms) are off the
// diagonal down here — a silent channel is a degenerate rectangle, not a common rate.
// (the orthographic corner has no ground-floor image at all: no cells to compare.)
for(let t=0;t<20;t++){
  const y=0.5+3*rnd();
  ok(Math.abs(M.defectOf(W-0.05,y))>1e-2,'d -> 0 flank is off the loop seam, t='+t);
  ok(Math.abs(M.defectOf(0.05,y))>1e-2,'c -> 0 flank is off the loop seam, t='+t);
}
ok(M.clampP(0,2).x===0.45&&M.clampP(W,2).x===W-0.45,
  'the drag clamps against the degenerate edges');
